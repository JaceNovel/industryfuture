...
export default function AdminCategoriesAddPage() {
  ...
  const createCategory = useMutation({
    mutationFn: async () => {
      ...
      return apiPost<Category>("/api/admin/categories", formData);
    },
    onSuccess: async () => {
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-8 md:px-6">
      <h1 className="text-3xl font-semibold tracking-tight">Ajouter une catégorie</h1>
      ...
      {createCategory.isError ? (
        <div className="mt-4 text-sm text-destructive">{(createCategory.error as Error).message}</div>
      ) : null}
      ...
      <div className="mt-8 flex items-center justify-end gap-3">
        ...
        <Button onClick={() => createCategory.mutate()} disabled={!name.trim() || createCategory.isPending}>
          Créer la catégorie
        </Button>
      </div>
    </main>
