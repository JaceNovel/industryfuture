...
async function handleAdmin(request: NextRequest, user: SessionUser, segments: string[]) {
  ...
  if (resource === "import-requests") {
    ...
    if (request.method === "GET" && segments[3] === "proof-pdf") {
      ...
      const pdf = await createImportRequestApprovalPdf(item);
      return new NextResponse(pdf, {
        status: 200,
        headers: {
          "Content-Type": "application/pdf",
          "Content-Disposition": `attachment; filename=service-premium-partenaire-${item.id}.pdf`,
        },
      });
    }
    ...
    if (request.method === "POST" && segments[3] === "notify") {
      const item = await prisma.importRequest.findUnique({
        where: { id: requestId },
        include: { payment: true, user: { select: { id: true, name: true, email: true } } },
      });
      if (!item) return fail("Not found", 404);
      if (!item.user?.email) return fail("No recipient email found for this request.", 422);
      if (!isEmailDeliveryConfigured()) return fail("Email delivery is not configured on the server.", 503);

      try {
        await notifyImportRequestUpdate(item);
      } catch (error) {
        console.error("import-request:manual-notify-failed", {
          requestId: item.id,
          error: error instanceof Error ? error.message : String(error),
        });
        return fail(error instanceof Error ? error.message : "Unable to send email notification.", 500);
      }
    ...
    if (request.method === "PATCH") {
      ...
      const shouldNotify = Boolean(
        item.user?.email && (
          ...
          payload.admin_price !== undefined ||
          payload.tracking_number !== undefined
        ) && (
          existing.status !== item.status ||
          Number(existing.admin_price ?? 0) !== Number(item.admin_price ?? 0) ||
          (existing.tracking_number ?? null) !== (item.tracking_number ?? null)
        ),
      );

      if (shouldNotify && ["accepted", "priced", "awaiting_payment", "processing", "shipped", "delivered"].includes(item.status)) {
        notifyImportRequestUpdate(item).catch((error) => {
          console.error("import-request:notify-failed", {
            requestId: item.id,
            error: error instanceof Error ? error.message : String(error),
          });
        });
